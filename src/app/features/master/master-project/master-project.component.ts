...
@Component({
  ...
  styleUrl: './master-project.component.css',
})
export class MasterProjectComponent {
  ...
  loadProjects(): void {
    ...
    this.apiService.get('/project-master', query).subscribe({
      next: (response) => {
        this.loading = false;
  ...
  goToDetail(row: any): void {
    ...
    void this.router.navigate(['/master-project', id]);
  }
  ...
  saveProject(form: NgForm): void {
    ...
    const request$ = this.apiService.post('/project-master', payload);
  ...
  deleteProject(row: any): void {
    ...
    this.apiService.delete(`/project-master/${id}`).subscribe({
      next: (response) => {
