...
@Component({
  selector: 'app-master-project-detail',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, NgbDatepickerModule],
  templateUrl: './master-project-detail.component.html',
  styleUrl: './master-project-detail.component.css',
})
export class MasterProjectDetailComponent implements OnInit {
  ...
  loadProjectDetail(): void {
    ...
    this.apiService.get(`/project-master/${this.projectId}`).subscribe({
      next: (response) => {
        this.loading = false;
  ...
  async loadOptions(): Promise<void> {
    ...
    try {
      const [clientResponse, projectTypeResponse, projectBilleableResponse, productResponse, userResponse] = await Promise.all([
 ...
 toggleManager(index: number): void {
  ...
  saveProject(form: NgForm): void {
    ...
    this.apiService.put(`/project-master/${this.projectId}`, payload).subscribe({
      next: (response) => {
        this.saving = false;
  ...
  deleteProject(): void {
    ...
    this.apiService.delete(`/project-master/${this.projectId}`).subscribe({
      next: () => {
        this.deleting = false;
        void this.router.navigate(['/master-project']);
      },
