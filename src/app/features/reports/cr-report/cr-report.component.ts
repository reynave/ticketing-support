...
@Component({
  ...
  styleUrl: './cr-report.component.css'
})
export class CrReportComponent implements OnInit {
  ...
  loadReport(): void {
    ...
    this.apiService.get('/adminReport/cr', query).subscribe({
      next: (response) => {
        this.loading = false;
  ...
  open(content: any, caseId: any): void {
    ...
    this.apiService.get('/adminReport/cr/detail', { id: this.selectedCaseId }).subscribe({
      next: (response) => {
